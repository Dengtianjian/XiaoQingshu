...
let functions = {
  async joinSchool(event) {
    ...
    let joinedSchoolLog = await DB.collection("user_joined_school")
      .where({
        _schoolid,
    ...
    let addResult = await DB.collection("user_joined_school")
      .add({
        data: {
          _schoolid,
          _userid: wxContext.OPENID,
  ...
  async updateSchoolStudents(event) {
    ...
    return await DB.collection("user_joined_school")
      ...
      .count()
      .then(async (res) => {
        return await DB.collection("school")
          .doc(_schoolid)
          .update({
            data: {
              students: res.total,
  ...
  async searchByKeyword(event) {
    ...
    let result=await DB.collection("school")
      .where({
        name: DB.RegExp({
          regexp: `.*${keyword}.*`,
          options: "ig",
